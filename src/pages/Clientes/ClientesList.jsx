...
const ClienteList = () => {
    ...
    const buscarCliente = async () => {
        ...
        try {
            ...
            const res = await fetchData(`/clientes/buscar?cedula=${valor}`)
    ...
    const confirmDelete = async () => {
        ...
        try {
            await fetchData(
                `/clientes/eliminar/${clienteToDelete._id}`,
                undefined,
                "DELETE"
            )
                            ...
                            {clientes.map(cliente => (
                                <tr key={cliente._id} className="border-b hover:bg-gray-50">
                                    ...
                                    <td className="p-3 text-center flex justify-center gap-2">
                                        <button
                                            onClick={() =>
                                                navigate(`/clientes/actualizar/${cliente._id}`)
                                            }
            ...
            {showModal && (
                <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
                    <div className="bg-gradient-to-br from-rose-100 via-orange-100 to-amber-100 rounded-2xl shadow-2xl p-6 w-96 border border-rose-200">
                        <h3 className="text-lg font-bold mb-4 text-rose-700">
